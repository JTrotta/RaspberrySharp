...
namespace RaspberrySharp.IO.GeneralPurpose
{
    ...
    public class GpioConnectionDriver : IGpioConnectionDriver
    {
        ...
        {
            MemoryMap.Close(gpioAddress, OP.BCM2835_BLOCK_SIZE);
        }
        ...
        public void Allocate(ProcessorPin pin, PinDirection direction)
        {
            ...
            SetPinMode(pin, direction == PinDirection.Input ? OP.BCM2835_GPIO_FSEL_INPT : OP.BCM2835_GPIO_FSEL_OUTP);
        ...
        public void SetPinResistor(ProcessorPin pin, PinResistor resistor)
        {
            ...
            switch (resistor)
            {
                case PinResistor.None:
                    pud = OP.BCM2835_GPIO_PUD_OFF;
                    break;
                case PinResistor.PullDown:
                    pud = OP.BCM2835_GPIO_PUD_DOWN;
                    break;
                case PinResistor.PullUp:
                    pud = OP.BCM2835_GPIO_PUD_UP;
                    break;
            ...
            HighResolutionTimer.Sleep(resistorSetDelay);
            WriteResistor(OP.BCM2835_GPIO_PUD_OFF);
        ...
        public void Wait(ProcessorPin pin, bool waitForUp = true, TimeSpan timeout = new TimeSpan())
        {
            ...
            while (true)
            {
                ...
                var waitResult = OP.epoll_wait(pinPoll.PollDescriptor, pinPoll.OutEventPtr, 1, (int)actualTimeout.TotalMilliseconds);
                if (waitResult > 0)
                {
                    if (Read(pin) == waitForUp)
                        break;
        ...
        public void Release(ProcessorPin pin)
        {
            ...
            SetPinMode(pin, OP.BCM2835_GPIO_FSEL_INPT);
        }
        ...
        private void InitializePoll(ProcessorPin pin)
        {
            lock (pinPolls)
            {
                ...
                var ev = new OP.epoll_event
                {
                    events = (OP.EPOLLIN | OP.EPOLLET | OP.EPOLLPRI),
                    data = new OP.epoll_data { fd = pinPoll.FileDescriptor }
                };

                pinPoll.InEventPtr = Marshal.AllocHGlobal(64);
                Marshal.StructureToPtr(ev, pinPoll.InEventPtr, false);

                var controlResult = OP.epoll_ctl(pinPoll.PollDescriptor, OP.EPOLL_CTL_ADD, pinPoll.FileDescriptor, pinPoll.InEventPtr);
                if (controlResult != 0)
        ...
        private void SetPinMode(ProcessorPin pin, uint mode)
        {
            ...
            var pinModeAddress = gpioAddress + (int)(OP.BCM2835_GPFSEL0 + 4 * ((int)pin / 10));

            var shift = 3 * ((int)pin % 10);
            var mask = OP.BCM2835_GPIO_FSEL_MASK << shift;
